...
/datum/admins/proc/drop_bomb()
	...
	switch(choice)
		if("CAS: Widow Maker")
			...
			addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(delayed_detonate_bomb), get_turf(usr.loc), 2, 4, 6, 0, 0, 0, 3), 1 SECONDS)
		if("CAS: Banshee")
			playsound(usr.loc, 'sound/machines/hydraulics_2.ogg', 70, TRUE)
			new /obj/effect/overlay/temp/blinking_laser (usr.loc)
			addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(delayed_detonate_bomb), get_turf(usr.loc), 2, 4, 7, 6, 7, 0, 3), 1 SECONDS)
		if("CAS: Keeper")
			playsound(usr.loc, 'sound/machines/hydraulics_2.ogg', 70, TRUE)
			new /obj/effect/overlay/temp/blinking_laser (usr.loc)
			addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(delayed_detonate_bomb), get_turf(usr.loc), 4, 5, 5, 6, 0, 0, 3), 1 SECONDS)
		if("CAS: Fatty")
			playsound(usr.loc, 'sound/machines/hydraulics_2.ogg', 70, TRUE)
			new /obj/effect/overlay/temp/blinking_laser (usr.loc)
			addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(delayed_detonate_bomb_fatty), get_turf(usr.loc)), 1 SECONDS)
		if("CAS: Napalm")
			playsound(usr.loc, 'sound/machines/hydraulics_2.ogg', 70, TRUE)
			new /obj/effect/overlay/temp/blinking_laser (usr.loc)
			addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(delayed_detonate_bomb_napalm), get_turf(usr.loc)), 1 SECONDS)
		if("Small Bomb")
...
/proc/delayed_detonate_bomb_fatty(turf/impact)
	...
	addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(delayed_detonate_bomb_fatty_final), impact), 3 SECONDS)
