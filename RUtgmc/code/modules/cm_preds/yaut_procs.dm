	...
	for(var/choice in secondaries)
		switch(choice)
			if(YAUTJA_GEAR_LAUNCHER)
				equip_to_slot_if_possible(new /obj/item/weapon/gun/energy/yautja/spike(src.loc), SLOT_IN_STORAGE, warning = TRUE)
			if(YAUTJA_GEAR_PISTOL)
				equip_to_slot_if_possible(new /obj/item/weapon/gun/energy/yautja/plasmapistol(src.loc), SLOT_IN_STORAGE, warning = TRUE)
			if(YAUTJA_GEAR_DISC)
				equip_to_slot_if_possible(new /obj/item/explosive/grenade/spawnergrenade/smartdisc(src.loc), SLOT_IN_STORAGE, warning = TRUE)
